 ...
 */
export function evalAst(ast: Expr, ctx: EvalContext): unknown {
  switch (ast.op) {
    case "lit":
      return (ast as { value: unknown }).value;

    case "ref": {
      const path = (ast as { path: string }).path;
      return ctx.get(path);
    }
    ...
    case "lt":
    case "lte": {
      const bin = ast as { left: Expr; right: Expr };
    ...
    case "and": {
      const andLeft = (ast as { left?: Expr; args?: Expr[] }).left;
      const andArgs = (ast as { left?: Expr; args?: Expr[] }).args;
      const andOperands = andArgs ?? (andLeft != null ? [andLeft, (ast as { right?: Expr }).right].filter(isExpr) : []);
    ...
    case "or": {
      const orLeft = (ast as { left?: Expr; args?: Expr[] }).left;
      const orArgs = (ast as { left?: Expr; args?: Expr[] }).args;
      const orOperands = orArgs ?? (orLeft != null ? [orLeft, (ast as { right?: Expr }).right].filter(isExpr) : []);
...
function compare(
  ...
  if (op === "eq") return looseEq(a, b);
  if (op === "neq") return !looseEq(a, b);
...
function compareValues(a: unknown, b: unknown): number | undefined {
  ...
  if (typeof a === "number" && typeof b === "string") return compareValues(a, Number(b));
  if (typeof a === "string" && typeof b === "number") return compareValues(Number(a), b);
  return undefined;
}
