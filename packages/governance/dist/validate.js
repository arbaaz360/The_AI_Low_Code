import { CANONICAL_WIDGET_TYPES, LEGACY_WIDGET_ALIASES, ALLOWED_ACTION_TYPES, ALLOWED_EVENT_NAMES, ALLOWED_SETVALUE_PATH_PREFIXES, ALLOWED_VALUE_BINDING_PREFIX, ALLOWED_OPTIONS_BINDING_PREFIXES, ALLOWED_EXPR_OPS, KNOWN_PROP_KEYS, MAX_EXPR_DEPTH, MAX_EXPR_NODE_COUNT, MAX_NODES, MAX_DOC_BYTES, } from "./constants.js";
function isExprLike(v) {
    return v != null && typeof v === "object" && "op" in v;
}
function measureExpr(expr, depth) {
    let maxD = depth;
    let count = 1;
    const children = [expr.left, expr.right, expr.then, expr.else];
    if (expr.args)
        children.push(...expr.args);
    for (const child of children) {
        if (!child || typeof child !== "object")
            continue;
        const sub = measureExpr(child, depth + 1);
        if (sub.maxDepth > maxD)
            maxD = sub.maxDepth;
        count += sub.nodeCount;
    }
    return { maxDepth: maxD, nodeCount: count };
}
function validateExprOps(expr, issues, path) {
...
function validateActions(actions, issues, basePath, nodeId) {
    for (let i = 0; i < actions.length; i++) {
        ...
        if (a.value && isExprLike(a.value)) {
            validateExprOps(a.value, issues, `${aPath}.value`);
        }
        if (a.message && isExprLike(a.message)) {
            validateExprOps(a.message, issues, `${aPath}.message`);
        }
        if (a.to && isExprLike(a.to)) {
            validateExprOps(a.to, issues, `${aPath}.to`);
        }
        if (a.actions)
            validateActions(a.actions, issues, `${aPath}.actions`, nodeId);
        if (a.then)
            validateActions(a.then, issues, `${aPath}.then`, nodeId);
        if (a.else)
            validateActions(a.else, issues, `${aPath}.else`, nodeId);
        if (a.onError)
            validateActions(a.onError, issues, `${aPath}.onError`, nodeId);
        if (a.onSuccess)
    ...
    for (const [nodeId, node] of nodeEntries) {
        ...
        if (n.bindings) {
            ...
            if (typeof bindings.options === "string") {
                if (!ALLOWED_OPTIONS_BINDING_PREFIXES.some((p) => bindings.options.startsWith(p))) {
        ...
        if (n.events) {
            for (const [evtName, actions] of Object.entries(n.events)) {
                ...
                if (Array.isArray(actions)) {
                    validateActions(actions, errors, `nodes.${nodeId}.events.${evtName}`, nodeId);
                }
            }
        }
    }
    if (d.pageEvents?.onLoad && Array.isArray(d.pageEvents.onLoad)) {
        validateActions(d.pageEvents.onLoad, errors, "pageEvents.onLoad");
    }
